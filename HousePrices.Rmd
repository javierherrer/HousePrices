...
cor_outl = cor(df_of_interest); cor_outl
require(corrplot)
par(mfrow=c(1,1))
...
A dfbeta value greater than `2 / sqrt(dim(house_prices)[1])`, indicates a large influence on the parameter estimate. Those values are temporarily removed from the dataset and a new model is reconstructed with it. This new model demontrates an improvement in the R-squared value from 0.91 to 0.97.
...
new_model <- lm(formula = (
    SalePrice ~ LotArea + YearBuilt + YearRemodAdd +
    MasVnrArea + BsmtFinSF1 + X1stFlrSF + X2ndFlrSF + BedroomAbvGr +
    KitchenAbvGr + Fireplaces + GarageArea + WoodDeckSF + OpenPorchSF +
    ScreenPorch + OverallQual + Neighborhood + ExterQual + BsmtQual +
    KitchenQual + GarageType + MSSubClass + f.YearBuilt
  ),
  data = reduced_data
)
