...
ll <- which(house_prices$BsmtFinSF1 + house_prices$BsmtFinSF2 + house_prices$BsmtUnfSF != house_prices$TotalBsmtSF);ll
# Histogram plotting
...
house_prices$GarageCars <- as.factor(house_prices$GarageCars)
# Plotting
...
```{r}
#GarageYrBlt
ll <- which(is.na(house_prices$GarageYrBlt));ll
...
# library(mice)
# res.imp <- mice(house_prices) # Imputation for both quantitative and qualitative variables
...
Strong Positive Correlations among Features (> 0.45):
...
```{r}
cor(house_prices[,c(2:14,24:34)],method="spearman")
```

### Univariate Outliers

Now the individuals are investigated. First the number of univariate outliers per individual are counted and added in a new variable called ‘univ_outl_count’. Looking at the 2 individuals with the most univariate outliers (>= 8) it can be concluded that they are all houses with a big living area and large LotArea. A correlation matrix confirms this as it shows a positive correlation to GrLiveArea, X1stFlrSF and LotArea.
```{r}
house_prices$univ_outl_count <- 0
house_prices$univ_outl_count[Boxplot(house_prices$LotFrontage, id = list(n=Inf))] = house_prices$univ_outl_count[Boxplot(house_prices$LotFrontage, id = list(n=Inf))] + 1
house_prices$univ_outl_count[Boxplot(house_prices$LotArea, id = list(n=Inf))] = house_prices$univ_outl_count[Boxplot(house_prices$LotArea, id = list(n=Inf))] + 1

house_prices$univ_outl_count[Boxplot(house_prices$YearBuilt, id = list(n=Inf))] = house_prices$univ_outl_count[Boxplot(house_prices$YearBuilt, id = list(n=Inf))] + 1

house_prices$univ_outl_count[Boxplot(house_prices$MasVnrArea, id = list(n=Inf))] = house_prices$univ_outl_count[Boxplot(house_prices$MasVnrArea, id = list(n=Inf))] + 1

house_prices$univ_outl_count[Boxplot(house_prices$BsmtFinSF1, id = list(n=Inf))] = house_prices$univ_outl_count[Boxplot(house_prices$BsmtFinSF1, id = list(n=Inf))] + 1

house_prices$univ_outl_count[Boxplot(house_prices$BsmtFinSF2, id = list(n=Inf))] = house_prices$univ_outl_count[Boxplot(house_prices$BsmtFinSF2, id = list(n=Inf))] + 1

house_prices$univ_outl_count[Boxplot(house_prices$BsmtUnfSF, id = list(n=Inf))] = house_prices$univ_outl_count[Boxplot(house_prices$BsmtUnfSF, id = list(n=Inf))] + 1

house_prices$univ_outl_count[Boxplot(house_prices$TotalBsmtSF, id = list(n=Inf))] = house_prices$univ_outl_count[Boxplot(house_prices$TotalBsmtSF, id = list(n=Inf))] + 1

house_prices$univ_outl_count[Boxplot(house_prices$X1stFlrSF, id = list(n=Inf))] = house_prices$univ_outl_count[Boxplot(house_prices$X1stFlrSF, id = list(n=Inf))] + 1

house_prices$univ_outl_count[Boxplot(house_prices$X2ndFlrSF, id = list(n=Inf))] = house_prices$univ_outl_count[Boxplot(house_prices$X2ndFlrSF, id = list(n=Inf))] + 1

house_prices$univ_outl_count[Boxplot(house_prices$LowQualFinSF, id = list(n=Inf))] = house_prices$univ_outl_count[Boxplot(house_prices$LowQualFinSF, id = list(n=Inf))] + 1

house_prices$univ_outl_count[Boxplot(house_prices$GrLivArea, id = list(n=Inf))] = house_prices$univ_outl_count[Boxplot(house_prices$GrLivArea, id = list(n=Inf))] + 1

house_prices$univ_outl_count[Boxplot(house_prices$GarageArea, id = list(n=Inf))] = house_prices$univ_outl_count[Boxplot(house_prices$GarageArea, id = list(n=Inf))] + 1

house_prices$univ_outl_count[Boxplot(house_prices$WoodDeckSF, id = list(n=Inf))] = house_prices$univ_outl_count[Boxplot(house_prices$WoodDeckSF, id = list(n=Inf))] + 1

house_prices$univ_outl_count[Boxplot(house_prices$OpenPorchSF, id = list(n=Inf))] = house_prices$univ_outl_count[Boxplot(house_prices$OpenPorchSF, id = list(n=Inf))] + 1

house_prices$univ_outl_count[Boxplot(house_prices$EnclosedPorch, id = list(n=Inf))] = house_prices$univ_outl_count[Boxplot(house_prices$EnclosedPorch, id = list(n=Inf))] + 1

house_prices$univ_outl_count[Boxplot(house_prices$X3SsnPorch, id = list(n=Inf))] = house_prices$univ_outl_count[Boxplot(house_prices$X3SsnPorch, id = list(n=Inf))] + 1

house_prices$univ_outl_count[Boxplot(house_prices$ScreenPorch, id = list(n=Inf))] = house_prices$univ_outl_count[Boxplot(house_prices$ScreenPorch, id = list(n=Inf))] + 1

house_prices$univ_outl_count[Boxplot(house_prices$PoolArea, id = list(n=Inf))] = house_prices$univ_outl_count[Boxplot(house_prices$PoolArea, id = list(n=Inf))] + 1

max(house_prices$univ_outl_count)

house_prices[which(house_prices$univ_outl_count >= 8),]

df_of_interest = house_prices[,c(3,11,27,30,10,14,34,45)]
...
# TODO: Do not forget to implement new variable definitions in the test sample.
...
Condes() is an R function from FactoMineR which is used to describe continous by quantitative variables and/or by qualitative variables. 
...
For qualitative variables, threee main features showed a high correlation with our target variable (R2 > 0.5 & p-value~0), which are OverallQuali (R2 = 0.70 & p-value = 0), Neighborhood (R2 = 0.58 & p-value ~ 0) and ExterQual (R2 = 0.50 & p-value ~ 0). The quality of the materials used to build the house is sigificantly important to determine the SalePrice. Neighborhood showed less correlation with SalePrice, but appeared to be an important factor to determine the SalePrice. Here we did not observe any factor with negative correlation to our target. 
```{r}
...
We initially built a model using only the numerical variables of our dataset. Here we observed that some of these variables presented NA's in the model (TotalBsmtSF, GrLivArea), meaning that we had multicollinearity in our model. To identify which variables had perfect correlation with each other in our dataset, we used cor() function. We identified that TotalBsmtSF had a very high correlation with X1stFlrSF (0.79) and GrLivArea was very correlated with TotRmsAbvGrd (0.82), so we decided to exclude them from our model. We also excluded Id and univ_outl_count because they were either non-informative or were created artificially to perform a given section of the project (univ_outl_count).

We created the second model with all these conditions described above. The linear regression model (m2) was fitted to predict the sale price (SalePrice) again. The model's intercept was not statistically significant (p = 0.35334), suggesting that the predicted sale price is not significantly different from zero when all other predictors are zero. Among the predictor variables, several were statistically significant with positive or negative coefficients. For instance, variables such as "YearBuilt," "YearRemodAdd," "BsmtFinSF1," "X1stFlrSF," and "X2ndFlrSF" had positive coefficients, indicating a positive relationship with sale price. On the other hand, variables like "BedroomAbvGr" and "KitchenAbvGr" had negative coefficients, suggesting a negative impact on sale price. The overall model explained a substantial portion of the variability in sale prices (Adjusted R-squared = 0.8391), and the F-statistic was highly significant (p < 2.2e-16), indicating that at least one of the predictors was significantly related to the sale price. However, we observed that some of the predictors were not statistically significant, so we performed a Stepwise (step()) to remove them.
By using step we were able to select a formula-based model by AIC. Here we were able to discard FullBath, MiscVal, YrSold, X3SsnPorch, BsmtFullBath, LowQualFinSF, PoolArea and BsmtHalfBath.
...
As a last step to create our model, we introduced all our categorical variables to the model and we run again step() to remove non-significant predictors. Here we discarded FireplaceQu, f.LotFrontage, f.LotArea, GarageFinish, MoSold, TotRmsAbvGrd, BsmtHalfBath, HalfBath, EnclosedPorch, LotFrontage, GarageType and Foundation. 

```{r}
numeric_variables <- sapply(house_prices, is.numeric)
m1 <- lm(SalePrice ~. ,data = house_prices[,numeric_variables])

summary(m1) # As we observe that TotalBsmtSF and GrLivArea are NA in the linear model, meaning that there's multicollinearity

# To check correlation between variables we apply cor()
variab <- cor(house_prices[,numeric_variables])

# Creating other model without these variables
...
m3 <- lm(formula = SalePrice ~ LotFrontage + LotArea + YearBuilt + 
    YearRemodAdd + MasVnrArea + BsmtFinSF1 + BsmtFinSF2 + BsmtUnfSF + 
    X1stFlrSF + X2ndFlrSF + BsmtHalfBath + HalfBath + BedroomAbvGr + 
    KitchenAbvGr + TotRmsAbvGrd + Fireplaces + GarageCars + GarageArea + 
    WoodDeckSF + OpenPorchSF + EnclosedPorch + ScreenPorch + 
    MoSold, data = house_prices[, selected])
summary(m3)

# Adding categorical variables
factors <- sapply(house_prices, is.factor)
total <- factors + selected
m4 <- lm(formula = SalePrice ~ LotFrontage + LotArea + YearBuilt + 
    YearRemodAdd + MasVnrArea + BsmtFinSF1 + BsmtFinSF2 + BsmtUnfSF + 
    X1stFlrSF + X2ndFlrSF + BsmtHalfBath + HalfBath + BedroomAbvGr + 
    KitchenAbvGr + TotRmsAbvGrd + Fireplaces + GarageCars + GarageArea + 
    WoodDeckSF + OpenPorchSF + EnclosedPorch + ScreenPorch + 
    MoSold + OverallQual+ Neighborhood + ExterQual + BsmtQual + KitchenQual + GarageFinish +
    FireplaceQu+ Foundation + GarageType + MSSubClass + f.LotFrontage + f.LotArea +
    f.YearBuilt, data = house_prices[, total == 1])

summary(m4)
step(m4)

# Final model
m5 <- lm(formula = SalePrice ~ LotArea + YearBuilt + YearRemodAdd + 
    MasVnrArea + BsmtFinSF1 + BsmtFinSF2 + BsmtUnfSF + X1stFlrSF + 
    X2ndFlrSF + BedroomAbvGr + KitchenAbvGr + Fireplaces + GarageCars + 
    GarageArea + WoodDeckSF + OpenPorchSF + ScreenPorch + OverallQual + 
    Neighborhood + ExterQual + BsmtQual + KitchenQual + GarageType + 
    MSSubClass + f.YearBuilt, data = house_prices[, total == 
    1])
