...
As a last step to create our model, we introduced all our categorical variables to the model and we run again step() to remove non-significant predictors. Here we discarded FireplaceQu, f.LotFrontage, f.LotArea, GarageFinish, MoSold, TotRmsAbvGrd, BsmtHalfBath, HalfBath, EnclosedPorch and LotFrontage. 
...
m7 <- lm(formula = SalePrice ~ LotFrontage + LotArea + YearBuilt + 
    YearRemodAdd + MasVnrArea + BsmtFinSF1 + X1stFlrSF + X2ndFlrSF + 
    BsmtHalfBath + HalfBath + BedroomAbvGr + KitchenAbvGr + TotRmsAbvGrd + 
    Fireplaces + GarageArea + WoodDeckSF + OpenPorchSF + EnclosedPorch + 
    ScreenPorch + MoSold, data = house_prices[, selected])
summary(m7)
# 
...
m8 <- lm(formula = SalePrice ~ LotFrontage + LotArea + YearBuilt + 
    ...
    BsmtHalfBath + HalfBath + BedroomAbvGr + KitchenAbvGr + TotRmsAbvGrd + 
    Fireplaces + GarageArea + WoodDeckSF + OpenPorchSF + EnclosedPorch + 
    ScreenPorch + MoSold + OverallQual+ Neighborhood + ExterQual + 
...
m9 <- lm(formula = SalePrice ~ LotArea + YearBuilt + YearRemodAdd + 
    MasVnrArea + BsmtFinSF1 + X1stFlrSF + X2ndFlrSF + BedroomAbvGr + 
    KitchenAbvGr + Fireplaces + GarageArea + WoodDeckSF + OpenPorchSF + 
    ScreenPorch + OverallQual + Neighborhood + ExterQual + BsmtQual + 
    KitchenQual + GarageType + MSSubClass + f.YearBuilt, data = house_prices[, 
    total == 1])
