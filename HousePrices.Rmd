...
length(Boxplot(house_prices$GarageCars, id = list(n=Inf)))
...
house_prices$univ_outl_count[Boxplot(house_prices$GarageCars, id = list(n=Inf))] = house_prices$univ_outl_count[Boxplot(house_prices$GarageCars, id = list(n=Inf))] + 1
...
#Prepare Test Data
test_data <- test_data[c("OverallQual", "Neighborhood", "ExterQual", "GrLivArea", "GarageCars", "GarageArea", "TotalBsmtSF", "X1stFlrSF", "YearBuilt", "YearRemodAdd")]
test_data$OverallQual <- factor(test_data$OverallQual)
test_data$Neighborhood <- factor(test_data$Neighborhood)
test_data$ExterQual <- factor(test_data$ExterQual)

cols <- c("OverallQual", "Neighborhood", "ExterQual")
levels_list <- list(
  1:10, # OverallQual
  c(
    "Blmngtn", "Blueste", "BrDale", "BrkSide", "ClearCr", "CollgCr", "Crawfor",
...
summary(test_data)
ll_na <- which(is.na(test_data$GarageCars) | is.na(test_data$GarageArea) | is.na(test_data$TotalBsmtSF));ll_na
#Discard observaions with NA's
...
# 
# # Assuming 'actual_values' are the true values from the test dataset
...
# 
# # Goodness of fit test H0: Model fits data
# deviance(final_model)
# pvalue <- 1 - pchisq(deviance(final_model),1);pvalue #I fail to reject the null hypothesis
```
